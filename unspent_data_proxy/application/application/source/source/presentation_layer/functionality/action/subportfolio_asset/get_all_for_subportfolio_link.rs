pub use crate::application_layer::functionality::action_processor::subportfolio_asset::get_all_for_subportfolio_link::SubportfolioAsset___GetAllForSubportfolioLink;
use crate::{
...
impl Action<SubportfolioAsset___GetAllForSubportfolioLink> {
    pub async fn run<'a>(body: &'a mut Body, parts: &'a Parts, route_parameters: &'a Params<'_, '_>, clickhouse_client: Client) -> Response {
        return Processor::<GeneralizedAction>::process::<_, _, _, _, _, _, _, Json>(
            ...
            ActionProcessor::<SubportfolioAsset___GetAllForSubportfolioLink>::process,
        )
        .await;
    }
}
