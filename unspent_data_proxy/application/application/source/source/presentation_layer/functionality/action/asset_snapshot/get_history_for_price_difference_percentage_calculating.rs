pub use crate::application_layer::functionality::action_processor::asset_snapshot::get_history_for_price_difference_percentage_calculating::AssetSnapshot___GetHistoryForPriceDifferencePercentageCalculating;
use crate::{
...
impl Action<AssetSnapshot___GetHistoryForPriceDifferencePercentageCalculating> {
    pub async fn run<'a>(body: &'a mut Body, parts: &'a Parts, route_parameters: &'a Params<'_, '_>, clickhouse_client: Client) -> Response {
        return Processor::<GeneralizedAction>::process::<_, _, _, _, _, _, _, Json>(
            ...
            ActionProcessor::<AssetSnapshot___GetHistoryForPriceDifferencePercentageCalculating>::process,
        )
        .await;
    }
}
