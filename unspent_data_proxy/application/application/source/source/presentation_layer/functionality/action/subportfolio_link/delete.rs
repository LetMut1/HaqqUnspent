pub use crate::application_layer::functionality::action_processor::subportfolio_link::delete::SubportfolioLink___Delete;
use crate::{
...
impl Action<SubportfolioLink___Delete> {
    pub async fn run<'a>(body: &'a mut Body, parts: &'a Parts, route_parameters: &'a Params<'_, '_>, clickhouse_client: Client) -> Response {
        return Processor::<GeneralizedAction>::process::<_, _, _, _, _, _, _, Json>(
            ...
            ActionProcessor::<SubportfolioLink___Delete>::process,
        )
        .await;
    }
}
