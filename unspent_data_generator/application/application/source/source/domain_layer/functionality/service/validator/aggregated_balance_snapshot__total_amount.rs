...
impl Validator<BaseBalanceSnapshot_Amount> {
    pub fn is_valid<'a>(base_balance_snapshot_amount: &'a BaseBalanceSnapshot_Amount) -> Result<bool, Auditor<Error>> {
        ...
        let balance_snapshot_amount_ = base_balance_snapshot_amount.0.as_str();

        return Ok(
            is_float_number && balance_snapshot_amount_ != "0" && balance_snapshot_amount_ != "0.0" && balance_snapshot_amount_ != ".0" && !balance_snapshot_amount_.contains('-'),
        );
    }
}
