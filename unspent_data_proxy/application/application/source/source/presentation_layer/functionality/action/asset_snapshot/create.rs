pub use crate::application_layer::functionality::action_processor::asset_snapshot::create::AssetSnapshot___Create;
use crate::{
...
impl Action<AssetSnapshot___Create> {
    pub async fn run<'a>(body: &'a mut Body, parts: &'a Parts, route_parameters: &'a Params<'_, '_>, clickhouse_client: Client) -> Response {
        return Processor::<GeneralizedAction>::process::<_, _, _, _, _, _, _, Json>(
            ...
            ActionProcessor::<AssetSnapshot___Create>::process,
        )
        .await;
    }
}
