pub use crate::application_layer::functionality::action_processor::subportfolio_asset::create_for_trackable_wallet::SubportfolioAsset___CreateForTrackableWallet;
use crate::{
...
impl Action<SubportfolioAsset___CreateForTrackableWallet> {
    pub async fn run<'a>(body: &'a mut Body, parts: &'a Parts, route_parameters: &'a Params<'_, '_>, clickhouse_client: Client) -> Response {
        return Processor::<GeneralizedAction>::process::<_, _, _, _, _, _, _, Json>(
            ...
            ActionProcessor::<SubportfolioAsset___CreateForTrackableWallet>::process,
        )
        .await;
    }
}
