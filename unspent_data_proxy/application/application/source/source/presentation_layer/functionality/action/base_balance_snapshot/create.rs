pub use crate::application_layer::functionality::action_processor::base_balance_snapshot::create::BaseBalanceSnapshot___Create;
use crate::{
...
impl Action<BaseBalanceSnapshot___Create> {
    pub async fn run<'a>(body: &'a mut Body, parts: &'a Parts, route_parameters: &'a Params<'_, '_>, clickhouse_client: Client) -> Response {
        return Processor::<GeneralizedAction>::process::<_, _, _, _, _, _, _, Json>(
            ...
            ActionProcessor::<BaseBalanceSnapshot___Create>::process,
        )
        .await;
    }
}
