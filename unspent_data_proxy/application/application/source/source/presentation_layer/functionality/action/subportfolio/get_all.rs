pub use crate::application_layer::functionality::action_processor::subportfolio::get_all::Subportfolio___GetAll;
use crate::{
...
impl Action<Subportfolio___GetAll> {
    pub async fn run<'a>(body: &'a mut Body, parts: &'a Parts, route_parameters: &'a Params<'_, '_>, clickhouse_client: Client) -> Response {
        return Processor::<GeneralizedAction>::process::<_, _, _, _, _, _, _, Json>(
            ...
            ActionProcessor::<Subportfolio___GetAll>::process,
        )
        .await;
    }
}
