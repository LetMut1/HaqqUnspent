pub use crate::application_layer::functionality::action_processor::subportfolio_asset::get_all_for_subportfolio::SubportfolioAsset___GetAllForSubportfolio;
use crate::{
...
impl Action<SubportfolioAsset___GetAllForSubportfolio> {
    pub async fn run<'a>(body: &'a mut Body, parts: &'a Parts, route_parameters: &'a Params<'_, '_>, clickhouse_client: Client) -> Response {
        return Processor::<GeneralizedAction>::process::<_, _, _, _, _, _, _, Json>(
            ...
            ActionProcessor::<SubportfolioAsset___GetAllForSubportfolio>::process,
        )
        .await;
    }
}
