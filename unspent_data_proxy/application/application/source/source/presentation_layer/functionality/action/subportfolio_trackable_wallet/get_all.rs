pub use crate::application_layer::functionality::action_processor::subportfolio_trackable_wallet::get_all::SubportfolioTrackableWallet___GetAll;
use crate::{
...
impl Action<SubportfolioTrackableWallet___GetAll> {
    pub async fn run<'a>(body: &'a mut Body, parts: &'a Parts, route_parameters: &'a Params<'_, '_>, clickhouse_client: Client) -> Response {
        return Processor::<GeneralizedAction>::process::<_, _, _, _, _, _, _, Json>(
            ...
            ActionProcessor::<SubportfolioTrackableWallet___GetAll>::process,
        )
        .await;
    }
}
