pub use crate::application_layer::functionality::action_processor::subportfolio_base_balance_snapshot::get_history_for_subportfolio_link::SubportfolioBaseBalanceSnapshot___GetHistoryForSubportfolioLink;
use crate::{
...
impl Action<SubportfolioBaseBalanceSnapshot___GetHistoryForSubportfolioLink> {
    pub async fn run<'a>(body: &'a mut Body, parts: &'a Parts, route_parameters: &'a Params<'_, '_>, clickhouse_client: Client) -> Response {
        return Processor::<GeneralizedAction>::process::<_, _, _, _, _, _, _, Json>(
            ...
            ActionProcessor::<SubportfolioBaseBalanceSnapshot___GetHistoryForSubportfolioLink>::process,
        )
        .await;
    }
}
