pub use crate::application_layer::functionality::action_processor::balance_snapshot::create::BalanceSnapshot___Create;
use crate::{
...
impl Action<BalanceSnapshot___Create> {
    pub async fn run<'a>(body: &'a mut Body, parts: &'a Parts, route_parameters: &'a Params<'_, '_>, clickhouse_client: Client) -> Response {
        return Processor::<GeneralizedAction>::process::<_, _, _, _, _, _, _, Json>(
            ...
            ActionProcessor::<BalanceSnapshot___Create>::process,
        )
        .await;
    }
}
